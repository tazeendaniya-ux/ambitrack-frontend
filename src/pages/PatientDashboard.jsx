...
export default function PatientDashboard() {
    ...
    try {
      ...
      if (emergencies.length > 0) {
        ...
        if (
          ...
          (!ambulanceLocation || !ambulanceLocation.lat)
        ) {
          setAmbulanceLocation({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (
        !latestEmergency ||
        latestEmergency.latitude == null ||
        latestEmergency.longitude == null
      )
        return;
  ...
  const distance =
    hasPatientCoordinates && ambulanceLocation
      ? getDistance(
  ...
  const eta =
    distance != null ? getETA(distance) : null;
      ...
      {latestEmergency && (
        <>
          ...
          <div style={cardStyle}>
            ...
            {latestEmergency.locationType === "manual" ? (
              <p>
                <strong>Pickup Address:</strong>{" "}
                {latestEmergency.manualLocation}
              </p>
            ) : (
              <p>
                <strong>Patient Location:</strong>{" "}
                {latestEmergency.latitude.toFixed(6)},
                {" "}
                {latestEmergency.longitude.toFixed(6)}
              </p>
            )}
